...
# `coworld build` pre-creates the output's parent; ci.yml on a fresh checkout
# does not, and every containment check that resolves the path by cd-ing into
# the parent then exits 1 (ecos, 2026-08-23). Make it first.
...
test -s "${dist}/cogmud_replay.wasm"
test -s "${dist}/cogmud_replay.js"

rm -rf "${output_dir}"
mkdir -p "${output_dir}/assets"
cp "${dist}/cogmud_replay.js" "${dist}/cogmud_replay.wasm" "${output_dir}/"
...
echo "cogmud replay viewer bundle: ${output_dir}"
