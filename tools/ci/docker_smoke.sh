...
# Substitute: cogmud, coworld-cogmud, 6.
#
...
#
# env:
#   SMOKE_IMAGE                image, if not given as $1        (coworld-cogmud:ci)
#   SMOKE_SLUG                 game slug                        (cogmud)
#   SMOKE_GAME_BIN             game entrypoint                  (/bin/cogmud)
#   SMOKE_PLAYER_BIN           player entrypoint                (/bin/cogmud-player)
...
image="${1:-${SMOKE_IMAGE:-coworld-cogmud:ci}}"
slug="${SMOKE_SLUG:-cogmud}"
...
# --------------------------------------------------------------------------
# Every PLAYER container must also exit 0. whisky's receiveMessage raises on a
# close frame and mummy's send only queues, so the game's quit(0) can outrun the
# flushed `done` frame and the player dies 1 -- intermittently, which is why
# certification catches it and a game-only exit check does not (raid 0.1.3,
# 2026-08-23). Players outlive the game by at most the close handshake.
# --------------------------------------------------------------------------
player_deadline=$((SECONDS + 60))
for ((slot = 0; slot < seats; slot++)); do
  while docker ps -q --filter "name=${prefix}-p${slot}" | grep -q .; do
    if (( SECONDS > player_deadline )); then
      echo "FAIL: player ${slot} still running 60s after the game exited" >&2
      dump_logs
      exit 1
    fi
    sleep 2
  done
  pexit="$(docker inspect -f '{{.State.ExitCode}}' "${prefix}-p${slot}")"
  if [ "${pexit}" != "0" ]; then
    echo "FAIL: player ${slot} container exited ${pexit} (expected 0)" >&2
    dump_logs
    exit 1
  fi
done
