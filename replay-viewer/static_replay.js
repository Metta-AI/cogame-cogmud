// Cogmud static replay shell: fetches the replay named by ?replay=<url>,
...
(function () {
  ...
  function start(module, bytes) {
    ...
    var ok = module._cm_load_replay(ptr, bytes.length);
    module._free(ptr);
    if (!ok) {
      fail(readString(module, module._cm_error_ptr(),
        module._cm_error_len()) || "wasm rejected the replay");
      return;
    }
    var payload = JSON.parse(
      readString(module, module._cm_payload_ptr(),
        module._cm_payload_len())
    );
    var loading = document.getElementById("loading");
    if (loading) loading.style.display = "none";
    document.documentElement.removeAttribute("data-replay-error");
    CogmudRenderer.attachReplay({
    ...
    // The renderer draws on its own animation frame, and two rAFs after
    // attachReplay is NOT enough: the sprites may still be loading, so the
    // bridge `ready` could beat the first drawn frame and an embedding host
    // (or viewer-check.yml) would sample a blank shell (eleusis, 2026-08-23).
    // Gate `ready` on the attribute the renderer sets on that first frame, so
    // `ready` always means a picture.
    var frames = 0;
    (function awaitFirstFrame() {
      if (document.documentElement.getAttribute("data-replay-loaded") ===
          "true") {
        tell("ready");
        return;
      }
      frames += 1;
      if (frames > 240) {
        fail("renderer never drew a frame");
        return;
      }
      window.requestAnimationFrame(awaitFirstFrame);
    })();
  }
  ...
  function load() {
    ...
    caption(attempt > 1 ? "RETRYING REPLAY… (attempt " + attempt + ")" :
      "LOADING REPLAY…");
    if (!modulePromise) {
      modulePromise = CogmudReplayModule().catch(function (error) {
